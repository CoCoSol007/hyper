...
class Main(ThreeDScene):
    def construct(self):
        ...
        point_M = always_redraw(
            lambda: Dot(
                point=[
                    x_tracker.get_value(),
                    y_tracker.get_value(),
                    c(x_tracker.get_value(), y_tracker.get_value()),
                ],
                color=RED,
            )
        )
        ...
        point_P = always_redraw(
            lambda: Dot(
                point=[
                    x_tracker.get_value()
                    / (c(x_tracker.get_value(), y_tracker.get_value()) + 1),
                    y_tracker.get_value()
                    / (c(x_tracker.get_value(), y_tracker.get_value()) + 1),
                    0,
                ],
        ...
        line_MP = always_redraw(
            lambda: Line(start=(0, 0, -1), end=point_M.get_center(), color=WHITE)
        )
