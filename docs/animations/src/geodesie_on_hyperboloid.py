...
class Main(ThreeDScene):
    def construct(self):
        ...
        poincare_disk = Circle(
            radius=1, color=GREEN, stroke_width=3, fill_color=GREEN, fill_opacity=0.1
        ).move_to(ORIGIN)
        ...
        geodesic_on_hyperboloid_1 = ParametricFunction(
            geodesic_hyperboloid_param_1,
            t_range=t_geodesic_range,
            color=RED,
            stroke_width=5,
        )
        geodesic_on_disk_1 = ParametricFunction(
            geodesic_disk_param_1,
            t_range=t_geodesic_range,
            color=YELLOW,
            stroke_width=5,
            use_smoothing=False,
        ...
        def geodesic_hyperboloid_param_2(t, phi):
            return np.array(
                [np.sinh(phi) * np.cosh(t), np.sinh(t), np.cosh(phi) * np.cosh(t)]
            )
        ...
        self.play(
            Create(geodesic_on_hyperboloid_1), Create(geodesic_on_disk_1), run_time=4
        )
        self.wait(1)
        ...
        point_on_hyperboloid_1.add_updater(
            lambda mob: mob.move_to(
                geodesic_hyperboloid_param_1(t_tracker_1.get_value())
            )
        )
        ...
        projection_line_1.add_updater(
            lambda mob: mob.put_start_and_end_on(
                point_on_hyperboloid_1.get_center(), point_on_disk_1.get_center()
            )
        )

        self.play(
            FadeIn(point_on_hyperboloid_1),
            FadeIn(point_on_disk_1),
            Create(projection_line_1),
        )
        self.play(
            t_tracker_1.animate.set_value(t_geodesic_range[1]),
            run_time=6,
            rate_func=linear,
        )
        self.play(
            FadeOut(point_on_hyperboloid_1),
            FadeOut(point_on_disk_1),
            FadeOut(projection_line_1),
            run_time=1,
        )
        ...
        self.play(
            Create(geodesic_on_hyperboloid_2), Create(geodesic_on_disk_2), run_time=4
        )
        self.wait(1)
        ...
        point_on_hyperboloid_2.add_updater(
            lambda mob: mob.move_to(
                geodesic_hyperboloid_param_2(t_tracker_2.get_value(), geodesic_2_phi)
            )
        )
        point_on_disk_2.add_updater(
            lambda mob: mob.move_to(
                geodesic_disk_param_2(t_tracker_2.get_value(), geodesic_2_phi)
            )
        )
        projection_line_2.add_updater(
            lambda mob: mob.put_start_and_end_on(
                point_on_hyperboloid_2.get_center(), point_on_disk_2.get_center()
            )
        )

        self.play(
            FadeIn(point_on_hyperboloid_2),
            FadeIn(point_on_disk_2),
            Create(projection_line_2),
        )
        self.play(
            t_tracker_2.animate.set_value(t_geodesic_range[1]),
            run_time=6,
            rate_func=linear,
        )
        self.play(
            FadeOut(point_on_hyperboloid_2),
            FadeOut(point_on_disk_2),
            FadeOut(projection_line_2),
            run_time=1,
        )
