...
class Main(Scene):
    def construct(self):
        ...
        shape_init_trans = Polygon([-1, 0, 0], [1, 0, 0], [0, 1, 0], color=BLUE, fill_opacity=0.7)
        ...
        label_vector = MathTex("\\vec{v}").next_to(arrow_translation.get_center(), UP, buff=0.1)
        ...
        self.wait(1.5)
        self.play(
            FadeOut(current_shape), FadeOut(arrow_translation),
            FadeOut(label_vector), FadeOut(title_translation)
        )
        self.wait(0.5)

        title_rotation = Text("Rotation", font_size=font_size_title).to_edge(UP)
        shape_init_rot = Polygon([-1, -1, 0], [1, -1, 0], [0, 0.5, 0], color=GREEN, fill_opacity=0.7)
        ...
        radius_arc = np.linalg.norm(current_shape.get_center() - center_rotation_point) * 0.5
        arc_rotation = Arc(
            radius=radius_arc,
            start_angle=Line(center_rotation_point, current_shape.get_center()).get_angle(),
        ...
        self.play(
            Rotate(current_shape, angle=rotation_angle, about_point=center_rotation_point),
            Create(arc_rotation)
        )
        self.wait(2)

        self.play(
            FadeOut(current_shape), FadeOut(center_visual),
            FadeOut(label_center), FadeOut(arc_rotation), FadeOut(title_rotation)
        )
        self.wait(0.5)

        title_reflection = Text("Reflection", font_size=font_size_title).to_edge(UP)
        shape_init_refl = Polygon([-1, 0, 0], [-3, 0, 0], [-1.5, 1.5, 0], color=PURPLE, fill_opacity=0.7)
        ...
        self.play(
            ...
            FadeOut(reflection_axis), FadeOut(label_axis),
