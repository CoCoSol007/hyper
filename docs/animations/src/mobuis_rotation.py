...
class Main(Scene):
    def construct(self):
        ...
        if abs(center_p_complex) >= 1:
            raise ValueError(
                f"Magnitude of w must be less than 1. Got {abs(center_p_complex)}"
            )
        ...
        def mobius_rotate_point(
            initial_point_R3: np.ndarray, center_p: complex, angle: float
        ) -> np.ndarray:
        ...
        center_p_marker = Dot(
            center_p_marker_pos, radius=dot_radius * 1.5, color=RED, z_index=1
        )
        ...
        angle_display_value.add_updater(
            lambda d: d.set_value(current_angle.get_value())
        )
        ...
        self.play(
            ...
            FadeOut(center_p_marker),
            FadeOut(center_p_label),
            FadeOut(title),
            FadeOut(angle_display_group),
        )
        self.wait(1)
