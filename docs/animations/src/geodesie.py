...
def get_poincare_geodesic(p1, p2, disk_radius=DISK_RADIUS):
    ...
    if abs(x1 * y2 - y1 * x2) < epsilon * disk_radius**2:
        ...
        try:
            ...
            print(
                f"p1={p1}, p2={p2}, center={center}, r={r}, angle1={angle1}, angle2={angle2}, sweep={sweep_angle}"
            )
            return Line(p1, p2, color=RED)
...
class Main(Scene):
    def construct(self):
        ...
        p1_label = Tex("P1", font_size=30).add_updater(
            lambda m: m.next_to(p1_dot, UR, buff=0.1)
        )
        p2_label = Tex("P2", font_size=30).add_updater(
            lambda m: m.next_to(p2_dot, UR, buff=0.1)
        )

        self.play(
            FadeIn(p1_dot, scale=0.5),
            FadeIn(p2_dot, scale=0.5),
            Write(p1_label),
            Write(p2_label),
        )
        self.wait(0.5)
        ...
        geodesic_mob = always_redraw(
            lambda: get_poincare_geodesic(
                p1_dot.get_center(), p2_dot.get_center(), disk_radius=DISK_RADIUS
            )
        )
