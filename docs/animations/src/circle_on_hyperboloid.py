...
class Main(ThreeDScene):
    def construct(self):
        ...
        def boost(point, alpha=0.5):
            ...
            return np.array(
                [
                    np.cosh(alpha) * x + np.sinh(alpha) * z,
                    y,
        ...
        proj_line1.add_updater(
            lambda mob: mob.put_start_and_end_on(
                point_hyp1.get_center(), point_disk1.get_center()
            )
        )
        ...
        self.play(
            FadeOut(point_hyp1), FadeOut(point_disk1), FadeOut(proj_line1), run_time=1
        )
        ...
        point_hyp2.add_updater(
            lambda mob: mob.move_to(
                boost(circle_hyperboloid(1, v_tracker2.get_value()), alpha)
            )
        )
        ...
        proj_line2.add_updater(
            lambda mob: mob.put_start_and_end_on(
                point_hyp2.get_center(), point_disk2.get_center()
            )
        )
        ...
        self.play(
            FadeOut(point_hyp2), FadeOut(point_disk2), FadeOut(proj_line2), run_time=1
        )
